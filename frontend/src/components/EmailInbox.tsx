...
const EmailRow = ({ email }) => {
    ...
    return (
        <>
            <ListItem onClick={handleClick} style={{ width: '100%', display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #f0f0f0', padding: '12px 0' }}>
                <Checkbox
                    checked={checked}
                ...
                />
                <ListItemText
                    primary={<Typography variant="subtitle1">{sender.full_name}</Typography>}
                />
                <ListItemText
                    primary={<Typography variant="body1">{subject}</Typography>}
                />
                <ListItemText
                    primary={<Typography variant="body1">{formatDate(sent_date)}</Typography>}
                />
            </ListItem>
            <Divider />
        </>
...
const EmailList = () => {
    ...
    return (
        ...
        status == 'failed' ? <p>Failed to load emails</p> :
